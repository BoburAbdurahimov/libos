...
export default function WardrobePage() {
  ...
  if (loading) return <div style={{ color: "#a8a29e" }}>Loading wardrobe…</div>;
    ...
    <div>
      ...
      {uploading && (
        <div style={{ background: "#fef3c7", borderRadius: 12, padding: 16, marginBottom: 20, display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{ color: "#b45309", fontWeight: 600 }}>🤖 AI is tagging your item…</div>
          <div style={{ color: "#78716c", fontSize: 13 }}>This takes a few seconds</div>
        </div>
      )}
      ...
      {garments.length === 0 && !uploading && (
        <div style={{ textAlign: "center", padding: "80px 20px", color: "#a8a29e" }}>
          ...
          <div style={{ fontSize: 20, fontWeight: 600, marginBottom: 8, color: "#78716c" }}>Your wardrobe is empty</div>
        ...
        {garments.map(g => (
          <div
            ...
            style={{
              ...
              border: `2px solid ${selected?.id === g.id ? "#b45309" : "transparent"}`,
              transition: "transform .15s, box-shadow .15s",
            }}
          >
            {g.imageUrl ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={g.imageUrl} alt={g.category} style={{ width: "100%", height: 220, objectFit: "cover", display: "block" }} />
            ) : (
              <div style={{ height: 220, background: "#f5f5f4", display: "flex", alignItems: "center", justifyContent: "center", color: "#a8a29e", fontSize: 13 }}>
                No image
              </div>
            )}
            <div style={{ padding: "10px 12px 12px" }}>
              <div style={{ fontWeight: 600, fontSize: 14, textTransform: "capitalize" }}>{g.category}</div>
              {g.subcategory && <div style={{ color: "#78716c", fontSize: 12, textTransform: "capitalize" }}>{g.subcategory}</div>}
              <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginTop: 6 }}>
                {g.colors.slice(0, 3).map(c => (
      ...
      {selected && (
        ...
        >
          <div style={{ background: "#fff", borderRadius: 20, maxWidth: 480, width: "100%", overflow: "hidden" }}>
            {selected.imageUrl && (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={selected.imageUrl} alt={selected.category} style={{ width: "100%", maxHeight: 380, objectFit: "cover", display: "block" }} />
            )}
            <div style={{ padding: 24 }}>
              <div style={{ fontSize: 20, fontWeight: 700, textTransform: "capitalize", marginBottom: 4 }}>{selected.category}</div>
              {selected.subcategory && <div style={{ color: "#78716c", textTransform: "capitalize", marginBottom: 12 }}>{selected.subcategory}</div>}

              <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 12 }}>
                {selected.colors.map(c => <span key={c} style={chip}>{c}</span>)}
                {selected.styleTags.map(s => <span key={s} style={{ ...chip, background: "#f0fdf4", color: "#16a34a" }}>{s}</span>)}
                {selected.season.map(s => <span key={s} style={{ ...chip, background: "#eff6ff", color: "#2563eb" }}>{s}</span>)}
              </div>

              {selected.aiDescription && (
                <p style={{ color: "#57534e", fontSize: 14, lineHeight: 1.6, margin: "0 0 16px" }}>{selected.aiDescription}</p>
              )}

              <div style={{ display: "flex", gap: 10 }}>
                <button onClick={() => setSelected(null)} style={{ flex: 1, padding: "10px 0", background: "#f5f5f4", border: "none", borderRadius: 10, cursor: "pointer", fontSize: 14 }}>
                  Close
                </button>
                <button onClick={() => del(selected.id)} style={{ flex: 1, padding: "10px 0", background: "#fef2f2", color: "#dc2626", border: "none", borderRadius: 10, cursor: "pointer", fontSize: 14, fontWeight: 500 }}>
                  Remove
                </button>
              </div>
            </div>
